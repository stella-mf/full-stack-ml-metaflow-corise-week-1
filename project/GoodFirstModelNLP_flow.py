...
class RFNLPFlow(FlowSpec):
    ...
    def rf_model(self):
        ...
        model.fit(train_text_features, self.traindf['label'])

        # Preprocess and transform the validation data as well
        val_text_features = tfidf_vectorizer.transform(self.valdf['review'])
